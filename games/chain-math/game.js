...
function generateQuestion() {
  ...
  const nums = [randInt(1, max), randInt(1, max), randInt(1, max)];
  let ops = [];
  ...
  // If answer < 0 or > 99, retry
  if (answer < 0 || answer > 99) return generateQuestion();

  const text = `${nums[0]} ${ops[0]} ${nums[1]} ${ops[1]} ${nums[2]}`;
  return { text, answer };
}
