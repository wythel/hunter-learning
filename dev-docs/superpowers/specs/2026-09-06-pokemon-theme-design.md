...
選「只留頂條」的關鍵理由不只是折衷：**這條頂條取代各遊戲現有的返回鍵與進度顯示**，
所以淨佔用空間接近零，卻能把主題一路延續進遊戲畫面。
...
已逐檔確認：**15 個遊戲畫面全部都有返回鍵**，所以 `onBack` 一律有對應的舊元素可取代，
「淨佔用空間接近零」的前提成立。但只有 10 個遊戲目前有進度顯示
（`chain-math`、`math-battle`、`math-mole`、`memory-flip`、`polar-day` 沒有），
因此 `progress` 與 `right` 皆為選填，未給值時該側留白，不得因此撐高頂條。
